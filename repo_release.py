...
ADDON=f'''{XML}\n<addon id="plugin.program.dragonmaxwizard" name="DragonMax Wizard" version="{VERSION}" provider-name="DragonMax RD"><requires><import addon="xbmc.python" version="3.0.0"/></requires><extension point="xbmc.python.pluginsource" library="default.py"><provides>executable</provides></extension><extension point="xbmc.addon.metadata"><summary>DragonMax V12 clean-runtime installer</summary><description>Protocol-3 installer: verified clean payload, selective rollback only for user configuration, existing active AuraMOD preservation, atomic DragonMax-owned writes, and Android-safe work storage.</description><platform>all</platform></extension></addon>'''
...
META={'dragonmax_manifest.json','dragonmax/install_manifest.json'}
PROTECTED=('userdata/Database/','userdata/Thumbnails/','userdata/temp/','addons/packages/','temp/','cache/','dragonmax_backups/')
...
BACKUP_USERDATA_PREFIXES=('userdata/keymaps/','userdata/advancedsettings.xml','userdata/addon_data/skin.auramod/','userdata/addon_data/script.autowidget/')
...
def should_backup(r):
 r=norm(r)
 return r.startswith('userdata/') and any(r.startswith(x) for x in BACKUP_USERDATA_PREFIXES)
def profile():
...
def verify(root,p):
 ...
 rows=manifest.get('files',[]); total=max(1,len(rows)); skipped=[]
 for i,row in enumerate(rows,1):
  ...
  if not os.path.isfile(f): raise RuntimeError('Payload manifest missing file: '+r)
  if os.path.getsize(f)!=int(row['size']) or sha(f).lower()!=str(row['sha256']).lower(): raise RuntimeError('Payload verification failed: '+r)
...
def active_auramod_present(home): return os.path.isfile(os.path.join(home,'addons','skin.auramod','addon.xml'))
def filter_for_device(home,fs):
 # Never rewrite an already-installed active skin while Kodi is running. The clean
 # payload still carries AuraMOD for fresh installs; existing AuraMOD is preserved.
 if not active_auramod_present(home): return fs
...
def preflight(home,fs):
 ...
 # Explicitly probe the two critical DragonMax destinations.
 for rel in ('addons/service.dragonmax.voice','userdata/addon_data/service.dragonmax.voice'):
  target=os.path.join(home,rel.replace('/',os.sep)); par=target if os.path.isdir(target) else os.path.dirname(target)
  os.makedirs(par,exist_ok=True); q=os.path.join(par,'.dragonmax-critical-probe')
  try:
   with open(q,'wb') as f:f.write(b'ok')
   os.remove(q)
  except Exception as e: raise RuntimeError('Critical DragonMax destination not writable: %s (%s)'%(par,e))
def backup(home,fs,root,p):
 ...
 for i,(r,_) in enumerate(fs,1):
  ...
  if os.path.isfile(d) and should_backup(r):
   ...
   except (PermissionError,OSError) as e: log('Skipping unreadable optional config backup '+r+': '+str(e),xbmc.LOGWARNING)
...
def apply(home,fs,p):
 ...
 for i,(r,s) in enumerate(fs,1):
  ...
  try: atomic(s,d)
   ...
   if owned(r) or r.startswith('addons/'):
    try: os.remove(d)
    except OSError: pass
    try: atomic(s,d)
...
def main():
 ...
 if int(build.get('install_protocol',0))<MIN_PROTOCOL: dlg.ok('DragonMax Wizard','Server payload has not passed the current clean-runtime protocol gate.'); return
 if not build.get('ready',False) and not dlg.yesno('DragonMax V12 DEVICE TEST','Protocol-3 clean-runtime staging build. Existing active AuraMOD is preserved; only user configuration is backed up; DragonMax components use atomic writes.\n\nRun device test install?'): return
 ...
 try:
  ...
  root=roots[0]; required=[os.path.join(root,'addons','skin.auramod','addon.xml'),os.path.join(root,'addons','service.dragonmax.voice','addon.xml'),os.path.join(root,'userdata'),os.path.join(root,'dragonmax','config'),os.path.join(root,'dragonmax','install_manifest.json')]
  if any(not os.path.exists(x) for x in required): raise RuntimeError('Payload missing required components')
  verify(root,p); fs=filter_for_device(home,files(root))
  if not fs: raise RuntimeError('No installable files')
  preflight(home,fs); o,c=backup(home,fs,br,p); apply(home,fs,p); pu(p,100,'Installation complete'); p.close(); dlg.ok('DragonMax V12 Installed','Device-test installation completed.\n\nFully exit Kodi and reopen it before validation.\n\nSelective rollback data: '+br)
...
def gates(s):
 tree=ast.parse(s)
 required=['MIN_PROTOCOL=3','filter_for_device','active_auramod_present','BACKUP_USERDATA_PREFIXES','selective rollback','atomic','preflight']
 for token in required:
  if token not in s: raise RuntimeError('Installer gate missing '+token)
 for node in ast.walk(tree):
  if isinstance(node,ast.Call) and isinstance(node.func,ast.Attribute) and node.func.attr in ('create','update') and isinstance(node.func.value,ast.Name) and node.func.value.id=='p' and len(node.args)>2:
   raise RuntimeError('Kodi DialogProgress API arity regression')
def publish(root:Path,out:Path):
 ...
 print('DragonMax Wizard 4.4.0 generated; clean-runtime protocol-3 and selective-rollback gates passed.')
