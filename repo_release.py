...
<window id="0">
  ...
  <controls>
    ...
    <control type="fixedlist" id="6100"><left>70</left><top>785</top><width>1780</width><height>245</height><orientation>horizontal</orientation><focusposition>0</focusposition><onup>5001</onup><ondown>6200</ondown>
...
def _patch_builder(frame, event, arg):
    ...
    if callable(generate_userdata) and not _builder_state['userdata']:
        ...
        def generate_unified_userdata():
            ...
            for token in ('DRAGONMAX','DragonMaxRealm','TRENDING MOVIES','plugin.program.dragonmaxportal','realm_change_'):
