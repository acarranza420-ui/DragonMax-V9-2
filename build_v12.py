#!/usr/bin/env python3
import io, os, json, math, random, shutil, struct, subprocess, sys, urllib.request, wave, zipfile, zlib
...
TARGET_MIN = 250 * 1024 * 1024
TARGET_MAX = 290 * 1024 * 1024
...
def png_bytes(w,h,tint,seed):
    rng = random.Random(seed)
    raw = bytearray()
    tr,tg,tb = tint
    for y in range(h):
        raw.append(0)
        row = bytearray(os.urandom(w*3))
        fade = int(55 * (y/max(1,h-1)))
        for i in range(0,len(row),3):
            row[i]   = (row[i]//3 + tr*2//3 + fade) & 255
            row[i+1] = (row[i+1]//3 + tg*2//3 + fade//2) & 255
            row[i+2] = (row[i+2]//3 + tb*2//3) & 255
        raw.extend(row)
    def chunk(tag,data):
...
def write_png(path,w,h,tint,seed):
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_bytes(png_bytes(w,h,tint,seed))
...
def copy_legacy():
    ...
    for p in tmp.rglob('*'):
        ...
        # preserve useful legacy-only configs/assets; V12 generated files overwrite later
        dst = STAGE/rel
...
def install_auramod():
    # Public Kodi 19+ AuraMOD fork, default branch Matrix.
    ...
    try:
        ...
        print('WARN AuraMOD fetch failed:',e)


def generate_media():
    art=STAGE/'artwork'; audio=STAGE/'audio'; startup=STAGE/'startup'
    # 48 realm wallpapers, 18 hero banners, 6 crests, 30 badges, 10 loading screens.
    idx=0
    for slug,name,tint in REALMS:
        for i in range(8):
            idx+=1; write_png(art/'wallpapers'/slug/f'{slug}_{i+1:02d}.png',1600,900,tint,10000+idx)
        for i in range(3):
            idx+=1; write_png(art/'hero_banners'/slug/f'{slug}_hero_{i+1:02d}.png',1600,600,tint,20000+idx)
        write_png(art/'realm_crests'/f'{slug}_crest.png',640,640,tint,30000+idx)
        for i,level in enumerate(['novice','adept','master','champion','legend']):
            write_png(art/'achievement_badges'/slug/f'{slug}_{level}.png',384,384,tint,40000+idx*10+i)
    for i in range(10):
        slug,name,tint=REALMS[i%6]
        write_png(art/'loading_screens'/f'loading_{i+1:02d}.png',1600,900,tint,50000+i)
    for i,(slug,name,tint) in enumerate(REALMS):
        write_png(art/'portal_graphics'/f'{slug}_portal.png',1280,720,tint,60000+i)
        write_png(art/'wizard_graphics'/f'{slug}_wizard.png',1280,720,tint,61000+i)
    # Startup splash plus compact original UI audio.
    write_png(startup/'dragonmax_static_splash.png',1920,1080,REALMS[0][2],70000)
...
def generate_userdata():
    ...
    menus={
      'layout':'netflix_first','home':['Dragon Portal','Continue Watching','Movies','TV Shows','Anime Universe','Martial Arts','Champion Guild','Office Consortium','Settings'],
      'portal':['Switch Realm','Resume Last Played','Audio Profile','Performance Mode','Maintenance','Backups','Updates','System Health','Admin']}
...
def make_zip():
    ...
    write()
    # Add real texture artwork until the payload reaches the Fire Stick target envelope.
    tex=STAGE/'artwork'/'texture_plates'; tex.mkdir(parents=True,exist_ok=True)
    i=1
    while BUILD.stat().st_size < TARGET_MIN and i <= 80:
        slug,name,tint=REALMS[i%6]
        write_png(tex/f'{slug}_texture_{i:02d}.png',1600,900,tint,80000+i)
        if i%4==0: write()
        i+=1
    write()
    mb=BUILD.stat().st_size/1024/1024
    print(f'V12 payload size: {mb:.2f} MiB')
    if BUILD.stat().st_size > TARGET_MAX:
        print('WARN payload exceeds preferred 290 MiB envelope')
...
def publish_repo_files():
    # Render serves the RC metadata together with the generated payload.
