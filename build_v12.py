...
def block_noise(seed,bx,by,limit):
    n=(seed*2654435761+bx*2246822519+by*3266489917)&0xffffffff; n^=n>>13; n=(n*1274126177)&0xffffffff; n^=n>>16; return n%max(1,limit)
def png_bytes(w,h,tint,seed,detail='normal'):
    raw=bytearray(); tr,tg,tb=tint; block=8 if detail=='normal' else 4; limit=36 if detail=='normal' else 72; blocks_x=(w+block-1)//block
    for y in range(h):
        raw.append(0); by=y//block; row_noise=[block_noise(seed,bx,by,limit) for bx in range(blocks_x)]
        for x in range(w):
            noise=row_noise[x//block]; wavev=int(22*math.sin((x+seed%97)/47.0)+16*math.cos((y+seed%53)/61.0)); fade=int(30*y/max(1,h-1)); raw.extend((max(0,min(255,tr+noise+wavev+fade)),max(0,min(255,tg+noise//2+wavev//2+fade//2)),max(0,min(255,tb+noise//3-wavev//3))))
    def chunk(tag,data):return struct.pack('>I',len(data))+tag+data+struct.pack('>I',zlib.crc32(tag+data)&0xffffffff)
    return b'\x89PNG\r\n\x1a\n'+chunk(b'IHDR',struct.pack('>IIBBBBB',w,h,8,2,0,0,0))+chunk(b'IDAT',zlib.compress(bytes(raw),7))+chunk(b'IEND',b'')
def write_png(path,w,h,tint,seed,detail='normal'):path.parent.mkdir(parents=True,exist_ok=True); path.write_bytes(png_bytes(w,h,tint,seed,detail))
...
def generate_media():
    ...
    for i,(slug,name,tint) in enumerate(REALMS):write_png(art/'portal_graphics'/f'{slug}_portal.png',1024,576,tint,60000+i); write_png(art/'wizard_graphics'/f'{slug}_wizard.png',1024,576,tint,61000+i)
    write_png(startup/'dragonmax_static_splash.png',1600,900,REALMS[0][2],70000); write_wav(audio/'startup_theme.wav',8.0,(55,110,165,220),.26); write_wav(audio/'portal_open.wav',2.0,(140,280,560),.22); write_wav(audio/'achievement.wav',1.4,(523,659,784,1046),.20); write_wav(audio/'ui_click.wav',.16,(900,1300),.17); write_wav(audio/'ui_back.wav',.22,(420,260),.17); write_wav(audio/'ui_select.wav',.30,(660,880,1320),.17); write_wav(audio/'error.wav',.45,(180,120),.18)
...
def generate_userdata():
    ...
    (u/'advancedsettings.xml').write_text('<advancedsettings><cache><buffermode>1</buffermode><memorysize>139460608</memorysize><readfactor>4.0</readfactor></cache></advancedsettings>',encoding='utf-8'); (u/'keymaps'/'dragonmax.xml').write_text('<keymap><global><keyboard><menu>ActivateWindow(Programs,plugin.program.dragonmaxwizard,return)</menu></keyboard></global></keymap>',encoding='utf-8')
def manifest():
...
def validate_quality():
    required=[STAGE/'addons'/'skin.auramod'/'addon.xml',STAGE/'addons'/'service.dragonmax.voice'/'addon.xml',STAGE/'addons'/'service.dragonmax.voice'/'service.py',STAGE/'addons'/'service.dragonmax.voice'/'dragon_memory.py',STAGE/'addons'/'service.dragonmax.voice'/'self_repair.py',STAGE/'userdata',STAGE/'dragonmax'/'config'/'menus.json',STAGE/'dragonmax'/'config'/'widgets.json',STAGE/'dragonmax'/'config'/'realms.json',STAGE/'dragonmax'/'config'/'performance.json',STAGE/'dragonmax'/'config'/'voice.json',STAGE/'dragonmax'/'install_manifest.json',STAGE/'startup'/'dragonmax_static_splash.png']+[STAGE/'addons'/aid/'addon.xml' for aid in BOOTSTRAP_PACKAGES]
...
def publish_repo_files():
    ...
    meta=json.loads((ROOT/'build.json').read_text(encoding='utf-8')); build=meta['builds'][0]; build['version']=RELEASE_VERSION; build['zip']=f'builds/DragonMax_V12_Unified_Build_Content-{RELEASE_VERSION}.zip'; build['last_built_size_mb']=round(BUILD.stat().st_size/1024/1024,2); build['sha256']=sha256_file(BUILD); build['install_protocol']=INSTALL_PROTOCOL; build['protected_runtime_paths']=['userdata/Database','userdata/Thumbnails','userdata/temp','addons/packages','userdata/guisettings.xml']; build['payload_policy']='clean runtime image; AuraMOD Omega 2.0.4 exact commit; bootstrap repositories bundled; official Kodi Omega direct dependencies verified; no inherited device userdata; no development metadata; fresh/existing-skin/rollback simulations passed'; build['auramod_version']='2.0.4'; build['auramod_commit']=AURAMOD_COMMIT; (OUT/'build.json').write_text(json.dumps(meta,indent=2),encoding='utf-8')
def main():clean(); copy_legacy(); reset_userdata(); sanitize_stage(); install_auramod(); install_bootstrap_addons(); install_dragonmax_addons(); generate_media(); generate_userdata(); sanitize_stage(); manifest(); payload_file_manifest(); make_zip(); publish_repo_files(); repo_release.publish(ROOT,OUT); print('DragonMax V12 launch-candidate distribution complete:',BUILD)
